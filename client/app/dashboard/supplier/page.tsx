"use client"
import { ConfirmDialog, EmptyState, ErrorState, LoadingState, PageHeader, RowActions } from "@/modules/components/shared";
import { useDeleteSupplier, useSuppliers } from "@/modules/hooks/useSupplier"
...
export default function SupplierPage(){
    ...
    return(
        <div>
          <PageHeader title="Supplier" createHref="/dashboard/supplier/create" createLabel="Add Supplier"/>
          <div className="bg-gray-800 rounded-xl shadow overflow-hidden border border-gray-700">
            {isLoading ? (<LoadingState message="Loading suppliers..."/>):isError? (<ErrorState message="Error loading suppliers"/>):!suppliers?.length? (<EmptyState message="No suppliers found"/>):(
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {suppliers?.map((supplier:any) => (
                      <tr key={supplier.id} className="hover:bg-gray-700/30 transition-colors">
                        ...
                        <td className="p-4 font-medium">{supplier.contactPerson}</td>
                        <td className="p-4 font-medium">{supplier.contactPersonPhone}</td>
                        ...
                        <td className="p-4">
                          <RowActions
                            ...
                            isDeleting={deleteMutation.isPending && confirmId === supplier.id}
                          />
                        </td>
                      </tr>
                    ))}
