...
import { useProduction, useUpdateProduction } from "@/modules/hooks/useProduction";
...
export default function EditProductionPage() {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-gray-400">Loading production data...</div>;
  }

  if (!production) {
    return <div className="p-8 text-center text-red-400">Production batch not found.</div>;
  }
  ...
  return (
    <div className="flex-1 p-6 text-gray-100 max-w-4xl mx-auto w-full">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Modify the status or raw material allocations for this run.</p>
      </div>
